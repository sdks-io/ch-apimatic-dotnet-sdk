...
public sealed class BackupBucketGetError : ApiError
{
    ...
    private BackupBucketGetError(Optional<V1OrganizationsServicesBackupBucket400Error1> v1OrganizationsServicesBackupBucket400Error1Value,
    ...
    private static BackupBucketGetError AsV1OrganizationsServicesBackupBucket400Error1(V1OrganizationsServicesBackupBucket400Error1 value) =>
        new(Optional<V1OrganizationsServicesBackupBucket400Error1>.Some(value), default, default);

    private static BackupBucketGetError AsV1OrganizationsServicesBackupBucket500Error1(V1OrganizationsServicesBackupBucket500Error1 value) =>
        new(default, Optional<V1OrganizationsServicesBackupBucket500Error1>.Some(value), default);

    private static BackupBucketGetError AsFallback(RawError value) =>
    ...
    internal static Task<BackupBucketGetError> Create(HttpResponseMessage response, CancellationToken ct) =>
...
internal sealed class BackupBucketGetErrorResponse : IErrorResponse<BackupBucketGetError>
{
    public static BackupBucketGetErrorResponse Instance { get; } = new();

    private BackupBucketGetErrorResponse()
    {
    }

    public Task<BackupBucketGetError> Map(HttpResponseMessage response, CancellationToken ct) =>
        BackupBucketGetError.Create(response, ct);
}
