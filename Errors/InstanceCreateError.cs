...
public sealed class InstanceCreateError : ApiError
{
    ...
    private InstanceCreateError(Optional<V1OrganizationsServices400Error1> v1OrganizationsServices400Error1Value,
    ...
    private static InstanceCreateError AsV1OrganizationsServices400Error1(V1OrganizationsServices400Error1 value) =>
        new(Optional<V1OrganizationsServices400Error1>.Some(value), default, default);

    private static InstanceCreateError AsV1OrganizationsServices500Error1(V1OrganizationsServices500Error1 value) =>
        new(default, Optional<V1OrganizationsServices500Error1>.Some(value), default);

    private static InstanceCreateError AsFallback(RawError value) =>
    ...
    internal static Task<InstanceCreateError> Create(HttpResponseMessage response, CancellationToken ct) =>
...
internal sealed class InstanceCreateErrorResponse : IErrorResponse<InstanceCreateError>
{
    public static InstanceCreateErrorResponse Instance { get; } = new();

    private InstanceCreateErrorResponse()
    {
    }

    public Task<InstanceCreateError> Map(HttpResponseMessage response, CancellationToken ct) =>
        InstanceCreateError.Create(response, ct);
}
