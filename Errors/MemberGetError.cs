...
public sealed class MemberGetError : ApiError
{
    ...
    private MemberGetError(Optional<V1OrganizationsMembers400Error1> v1OrganizationsMembers400Error1Value,
    ...
    private static MemberGetError AsV1OrganizationsMembers400Error1(V1OrganizationsMembers400Error1 value) =>
        new(Optional<V1OrganizationsMembers400Error1>.Some(value), default, default);

    private static MemberGetError AsV1OrganizationsMembers500Error1(V1OrganizationsMembers500Error1 value) =>
        new(default, Optional<V1OrganizationsMembers500Error1>.Some(value), default);

    private static MemberGetError AsFallback(RawError value) =>
    ...
    internal static Task<MemberGetError> Create(HttpResponseMessage response, CancellationToken ct) =>
...
internal sealed class MemberGetErrorResponse : IErrorResponse<MemberGetError>
{
    public static MemberGetErrorResponse Instance { get; } = new();

    private MemberGetErrorResponse()
    {
    }

    public Task<MemberGetError> Map(HttpResponseMessage response, CancellationToken ct) =>
        MemberGetError.Create(response, ct);
}
