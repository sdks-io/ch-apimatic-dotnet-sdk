...
public sealed class OrganizationUpdateError : ApiError
{
    ...
    private OrganizationUpdateError(Optional<V1Organizations400Error1> v1Organizations400Error1Value,
    ...
    private static OrganizationUpdateError AsV1Organizations400Error1(V1Organizations400Error1 value) =>
        new(Optional<V1Organizations400Error1>.Some(value), default, default);

    private static OrganizationUpdateError AsV1Organizations500Error1(V1Organizations500Error1 value) =>
        new(default, Optional<V1Organizations500Error1>.Some(value), default);

    private static OrganizationUpdateError AsFallback(RawError value) =>
    ...
    internal static Task<OrganizationUpdateError> Create(HttpResponseMessage response, CancellationToken ct) =>
...
internal sealed class OrganizationUpdateErrorResponse : IErrorResponse<OrganizationUpdateError>
{
    public static OrganizationUpdateErrorResponse Instance { get; } = new();

    private OrganizationUpdateErrorResponse()
    {
    }

    public Task<OrganizationUpdateError> Map(HttpResponseMessage response, CancellationToken ct) =>
        OrganizationUpdateError.Create(response, ct);
}
