...
|  --- | --- | --- | --- |
| `DisplayType` | `string` | Required, Constant | Display type discriminator. Must be "pie" for pie charts.<br><br>**Value**: `"pie"` |
...
| `GroupBy` | `string` | Optional | Field expression to group results by (one slice per group value). |
| `OrderBy` | `string` | Optional | Optional custom SQL ORDER BY expression (raw SQL). Overrides the default value-descending ordering and, when combined with "limit", controls which slices are kept. |
...
ClickStackPieBuilderChartConfig clickStackPieBuilderChartConfig = new ClickStackPieBuilderChartConfig
{
    DisplayType = "pie",
