...
# SDK map — OpenAPI spec for ClickHouse Cloud (.NET)
...
| Root namespace | `OpenApiSpecForClickHouseCloud` |
...
All `Source` paths on this map and its sub-pages are relative to the **SDK root** — the directory holding this file and `OpenApiSpecForClickHouseCloud.csproj` — never to the page that carries them. Open them as-is from the SDK root, from any page; if the SDK sits under a subdirectory of a larger repo, prefix that subdirectory.
...
var options =
    new OpenApiSpecForClickHouseCloudClientOptions
    {
...
var client = new OpenApiSpecForClickHouseCloudClient(httpClient, options);
```

DI alternative (`services.AddOpenApiSpecForClickHouseCloudClient`):

```csharp
services.AddOpenApiSpecForClickHouseCloudClient(options =>
    {
        options.BasicAuth =
...
Every API group is a property on the client (e.g. `client.ApiKeys`). Source: `OpenApiSpecForClickHouseCloudClient.cs`. The only constructor is `OpenApiSpecForClickHouseCloudClient(HttpClient httpClient, OpenApiSpecForClickHouseCloudClientOptions options)`.

All `OpenApiSpecForClickHouseCloudClientOptions` properties (source: `OpenApiSpecForClickHouseCloudClientOptions.cs`):
...
`ServerEnvironment` (source: `Servers/ServerEnvironment.cs`, namespace `OpenApiSpecForClickHouseCloud.Servers`)
`BasicAuthCredentials` (source: `Core/Authentication/Basic/BasicAuthCredentials.cs`, namespace `OpenApiSpecForClickHouseCloud.Core.Authentication.Basic`)

`RetryOptions` members (namespace `OpenApiSpecForClickHouseCloud.Core.Configuration` — add `using OpenApiSpecForClickHouseCloud.Core.Configuration;`; source: `Core/Configuration/RetryOptions.cs`; all members are `required`, so build a full instance or start from `RetryOptions.Default()`):
...
Operations are **throw-based**. On an error status the SDK throws `SdkException<TError>` (`Core/Exceptions/SdkException.cs`, namespace `OpenApiSpecForClickHouseCloud.Core.Exceptions`) exposing `.Error` of type `TError`. There are two cases:

- **Case A — typed error.** `TError` is a generated `…Error : ApiError` class (namespace `OpenApiSpecForClickHouseCloud.Errors`) with status-specific `TryGet…(out …)` accessors (each returns `true` when that shape is present) plus the inherited `TryGetRawError(out RawError)` fallback. The operation blocks name the exact `TryGet…` methods and the HTTP status each maps to.
- **Case B — raw error.** `TError` is `RawError` (`Core/ErrorResponse/RawError.cs`, namespace `OpenApiSpecForClickHouseCloud.Core.ErrorResponse`): `StatusCode: HttpStatusCode` · `ReadAsBytes(): ReadOnlyMemory<byte>` · `ReadAsString(): string` · `ReadAsJson<T>(): T?`.
...
| --- | --- | --- |
| `ApiError` — abstract base of the 157 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
...
**No-throw (`…Result`) variants: absent across this SDK** — every operation is throw-only. Of **157 operations**, **157 are Case A (typed)** and **0 are Case B (raw)**.

---

## Operations — by controller (14 groups, 157 operations)
...
| --- | --- | --- |
| `ApiKeys` | 5 | [map/operations/ApiKeys.md](map/operations/ApiKeys.md) |
| `BackupApi` | 8 | [map/operations/BackupApi.md](map/operations/BackupApi.md) |
| `Billing` | 3 | [map/operations/Billing.md](map/operations/Billing.md) |
| `ClickPipes` | 18 | [map/operations/ClickPipes.md](map/operations/ClickPipes.md) |
| `ClickStack` | 30 | [map/operations/ClickStack.md](map/operations/ClickStack.md) |
| `OrganizationApi` | 11 | [map/operations/OrganizationApi.md](map/operations/OrganizationApi.md) |
| `Postgres` | 17 | [map/operations/Postgres.md](map/operations/Postgres.md) |
| `Prometheus` | 5 | [map/operations/Prometheus.md](map/operations/Prometheus.md) |
| `QueryApiEndpoints` | 5 | [map/operations/QueryApiEndpoints.md](map/operations/QueryApiEndpoints.md) |
| `RoleManagement` | 5 | [map/operations/RoleManagement.md](map/operations/RoleManagement.md) |
| `ServiceApi` | 26 | [map/operations/ServiceApi.md](map/operations/ServiceApi.md) |
| `SnapshotApi` | 4 | [map/operations/SnapshotApi.md](map/operations/SnapshotApi.md) |
| `UdfApi` | 12 | [map/operations/UdfApi.md](map/operations/UdfApi.md) |
| `UserManagement` | 8 | [map/operations/UserManagement.md](map/operations/UserManagement.md) |
...
| --- | --- | --- |
| Records (plain `record` data models) | 819 | `Models/` |
| Unions (`AnyOf`) — variant factories + `TryGet…` | 70 | `Models/AnyOf/` |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — C# member names + wire values | 143 | `Models/Enums/` |
| Typed error classes (`: ApiError`, one per Case A operation) | 157 | `Errors/` |
...
| Contents | Namespace |
| --- | --- |
| Client & options (root) | `OpenApiSpecForClickHouseCloud` |
| Operation controllers (`Api/`) | `OpenApiSpecForClickHouseCloud.Api` |
| Records (`Models/`) | `OpenApiSpecForClickHouseCloud.Models` |
| Enums (`Models/Enums/`) | `OpenApiSpecForClickHouseCloud.Models.Enums` |
| AnyOf unions (`Models/AnyOf/`) | `OpenApiSpecForClickHouseCloud.Models.AnyOf` |
| Error classes (`Errors/`) | `OpenApiSpecForClickHouseCloud.Errors` |

---

## Servers & auth

**Basic auth.** Set `options.BasicAuth = new BasicAuthCredentials { Username = …, Password = … }`. Use key ID and key secret obtained in ClickHouse Cloud console: https://clickhouse.com/docs/cloud/manage/openapi
...
| --- | --- | --- |
| `Default` | `https://api.clickhouse.cloud` | `options.Server.Default.Production.BaseUrl` |
